...
use scale_info::{
    form::PortableForm, PortableRegistry, TypeDefArray, TypeDefBitSequence, TypeDefCompact,
    TypeDefComposite, TypeDefPrimitive, TypeDefSequence, TypeDefTuple, TypeDefVariant, TypeDef
};
use crate::visitor::{
    Visitor,
    DecodeError,
...
pub fn decode<'a, V: Visitor>(
    ...
    match ty.type_def() {
        ...
        TypeDef::Compact(inner) => {
            decode_compact_value(data, inner, types, visitor)
        },
...
fn decode_bit_sequence_value<V: Visitor>(
	...
	let (store, order) = get_bitsequence_details(ty, types).map_err(DecodeError::BitSequenceError)?;
